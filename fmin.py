...
def gradient_descent(x, f, grad, tol=1e-6, alpha=[], line_search=ls.quadratic_fit_search):
    ...
    while np.linalg.norm(gradx) > tol:
        ...
        if not got_alpha:
            def f_aux(a): return f(x - a*gradx)
            bracket = ls.bracket(0, f_aux)
            alpha = line_search(f_aux, bracket[0], bracket[1], tol=tol)
...
if __name__ == "__main__":
    ...
    def f(x): return (100*(x[1] - x[0]**2)**2 + (x[0]-1)**2)

    def grad(x): return [-400*(x[1]-x[0]**2) *
                         x[0] + 2*(x[0]-1), 200*(x[1]-x[0]**2)]

    def H(x): return [[-400*x[1]+1200*x[0]**2, -400*x[0]], [-400*x[0], 200]]
